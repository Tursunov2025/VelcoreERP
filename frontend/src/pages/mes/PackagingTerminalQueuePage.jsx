...
export default function PackagingTerminalQueuePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [queue, dashboard] = await Promise.all([
        api.mesPackagingQueue(),
        api.mesPackagingDashboard(),
      ]);
      setData({
        jobs: queue.jobs || [],
        ...
        total_packages_today: dashboard.total_packages_today ?? 0,
      });
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="pb-24">
      <PageHeader
        title={t("mes.packagingTerminal")}
        subtitle={t("mes.packagingQueueSubtitle")}
        actions={
          <button
            type="button"
            onClick={load}
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
        <StatCard label={t("mes.packagingWaitingJobs")} value={data.waiting_jobs} />
        <StatCard label={t("mes.packagingActiveJobs")} value={data.active_jobs} accent="#d97706" />
        <StatCard label={t("mes.packagingCompletedToday")} value={data.completed_today} accent="#16a34a" />
        <StatCard label={t("mes.packagingPackagesToday")} value={data.total_packages_today} accent="#7c3aed" />
      </div>
      ...
      {data.jobs.length === 0 && !loading ? (
        ...
        <div className="space-y-3">
          {data.jobs.map((job) => (
            <Link
              key={job.id}
              to={`/mes/terminal/packaging/jobs/${job.id}`}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-[var(--brand-muted)]">
                    {job.packaging_step?.stage_name || t("mes.packagingTerminal")}
                  </p>
                </div>
                <span
