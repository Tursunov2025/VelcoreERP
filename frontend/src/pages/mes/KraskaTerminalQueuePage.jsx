...
const STEP_STATE_CLASS = {
  ...
  drying: "bg-purple-100 text-purple-900",
};
...
export default function KraskaTerminalQueuePage() {
  ...
  return (
    <div className="pb-24">
      <PageHeader
        title={t("mes.kraskaTerminal")}
        subtitle={t("mes.kraskaQueueSubtitle")}
        actions={
          <button
            type="button"
            onClick={load}
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
        <StatCard label={t("mes.kraskaWaitingJobs")} value={data.waiting_jobs} />
        <StatCard label={t("mes.kraskaActiveJobs")} value={data.active_jobs} accent="#d97706" />
        <StatCard label={t("mes.kraskaDryingJobs")} value={data.drying_jobs} accent="#9333ea" />
        <StatCard label={t("mes.kraskaCompletedToday")} value={data.completed_today} accent="#16a34a" />
      </div>
      ...
      {data.jobs.length === 0 && !loading ? (
        ...
        <div className="space-y-3">
          {data.jobs.map((job) => (
            <Link
              key={job.id}
              to={`/mes/terminal/kraska/jobs/${job.id}`}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-[var(--brand-muted)]">
                    {job.paint_metadata?.color_name || job.paint_step?.stage_name}
                  </p>
                </div>
                <span
                ...
                >
                  {t(`mes.kraskaStep_${job.step_state}`)}
                </span>
              </div>
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-[var(--brand-muted)]">
                  {job.paint_metadata?.ral_code || "—"}
                </span>
