...
export default function LoginPage({
  ...
  const handleSubmit = async (event) => {
    ...
    await onLogin(loginUser, loginPassword);
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#f5f6fa] p-4">
      <div className="w-full max-w-md rounded-[40px] bg-white p-8 shadow-2xl md:p-10">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="login-user" className="mb-2 block text-sm font-medium text-gray-600">
              Foydalanuvchi
            </label>
            <select
              id="login-user"
              ...
              className="w-full rounded-2xl border px-5 py-4 outline-none focus:ring-2 focus:ring-black disabled:bg-gray-100"
            >
              <option value="">
                {loading ? "Yuklanmoqda..." : "User tanlang"}
              </option>
              {users.map((user) => (
            ...
            {usersError && (
              <p className="mt-2 text-sm text-red-500">{usersError}</p>
            )}
          </div>

          <div>
            <label htmlFor="login-password" className="mb-2 block text-sm font-medium text-gray-600">
              Parol
            </label>
            <input
              id="login-password"
          ...
          <button
            ...
            className="w-full text-sm text-gray-500 underline-offset-2 hover:text-black hover:underline"
          >
