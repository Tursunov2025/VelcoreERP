from typing import Optional

from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel, Field
from sqlalchemy.orm import Session

from auth.deps import get_current_user
from database import get_db
from models import User
from services.mes_jobs import load_job
from services.mes_kraska_terminal import (
    accept_paint_job,
    complete_paint_job,
    get_paint_stages,
    list_paint_queue,
    paint_stage_ids,
    kraska_dashboard,
    send_to_drying,
    serialize_terminal_job,
    start_paint_job,
    update_paint_metadata,
    update_paint_quantities,
)
from services.permissions import user_has_permission

router = APIRouter(prefix="/mes/terminal/kraska", tags=["mes-terminal-kraska"])


class BomQuantityItem(BaseModel):
    bom_line_id: int
    painted_quantity: Optional[float] = Field(None, ge=0)
    accepted_quantity: Optional[float] = Field(None, ge=0)
    rejected_quantity: Optional[float] = Field(None, ge=0)


class QuantitiesUpdate(BaseModel):
    lines: list[BomQuantityItem]


class PaintMetadataUpdate(BaseModel):
    color_name: Optional[str] = None
    ral_code: Optional[str] = None
    paint_type: Optional[str] = None
    batch_number: Optional[str] = None


def _require_kraska_terminal(db: Session, user: User) -> None:
    if user_has_permission(db, user, "mes_terminal_kraska"):
        return
    raise HTTPException(status_code=403, detail="Permission required: mes_terminal_kraska")


def _paint_stages_or_503(db: Session):
    stages = get_paint_stages(db)
    if not stages:
        raise HTTPException(status_code=503, detail="Paint production stages are not configured")
    return stages


@router.get("/dashboard")
def kraska_dashboard_endpoint(
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    stages = _paint_stages_or_503(db)
    ids = {s.id for s in stages}
    return {
        "stages": [{"id": s.id, "name": s.name, "department": s.department} for s in stages],
        **kraska_dashboard(db, ids),
    }


@router.get("/queue")
def kraska_queue(
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    stages = _paint_stages_or_503(db)
    ids = {s.id for s in stages}
    return {
        "stages": [{"id": s.id, "name": s.name, "department": s.department} for s in stages],
        "jobs": list_paint_queue(db, ids),
    }


@router.get("/jobs/{job_id}")
def kraska_job_detail(
    job_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return serialize_terminal_job(job, ids, include_bom=True)


@router.post("/jobs/{job_id}/accept")
def kraska_accept_job(
    job_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    try:
        accept_paint_job(db, job, ids, user.username)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    return serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)


@router.post("/jobs/{job_id}/start")
def kraska_start_job(
    job_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    try:
        start_paint_job(db, job, ids, user.username)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    return serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)


@router.post("/jobs/{job_id}/drying")
def kraska_send_to_drying(
    job_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    try:
        send_to_drying(db, job, ids, user.username)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    return serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)


@router.post("/jobs/{job_id}/complete")
def kraska_complete_job(
    job_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    try:
        complete_paint_job(db, job, ids, user.username)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    return serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)


@router.put("/jobs/{job_id}/paint-metadata")
def kraska_update_paint_metadata(
    job_id: int,
    data: PaintMetadataUpdate,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    update_paint_metadata(
        db,
        job,
        user.username,
        color_name=data.color_name,
        ral_code=data.ral_code,
        paint_type=data.paint_type,
        batch_number=data.batch_number,
    )
    db.commit()
    return serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)


@router.put("/jobs/{job_id}/quantities")
def kraska_update_quantities(
    job_id: int,
    data: QuantitiesUpdate,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    _require_kraska_terminal(db, user)
    ids = paint_stage_ids(db)
    job = load_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    if not data.lines:
        raise HTTPException(status_code=400, detail="No quantity lines provided")
    try:
        payload_lines = [item.model_dump(exclude_none=True) for item in data.lines]
        auto_completed = update_paint_quantities(db, job, ids, user.username, payload_lines)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
    result = serialize_terminal_job(load_job(db, job_id), ids, include_bom=True)
    result["auto_completed"] = auto_completed
    return result
